...
from openocto.wizard_data import (
    ...
    QUICK_DEFAULTS,
    SILERO_SPEAKERS_RU as _SILERO_SPEAKERS_RAW,
    TTS_VOICES_EN as _TTS_VOICES_RAW,
    WAKE_WORD_OPTIONS as _WAKE_WORD_OPTIONS_RAW,
    detect_primary_lang,
    is_ollama_installed as _is_ollama_installed,
...
# CLI-decorated versions with star emoji for recommended items
TTS_VOICES_EN = [
    (k, f"{v}  {STAR} recommended" if i == 0 else v)
    for i, (k, v) in enumerate(_TTS_VOICES_RAW)
]

WAKE_WORD_OPTIONS = [
    (k, f"{v}  {STAR} recommended" if i == 0 else v)
    for i, (k, v) in enumerate(_WAKE_WORD_OPTIONS_RAW)
]

SILERO_SPEAKERS_RU = [
    (k, f"{v}  {STAR} recommended" if i == 0 else v)
    for i, (k, v) in enumerate(_SILERO_SPEAKERS_RAW)
]

_detect_primary_lang = detect_primary_lang
...
def _write_config(backend: str, api_key: str, model_size: str, voice_en: str, voice_ru: str,
    ...
    click.secho(f"  {CHECK} Config saved to {path}", fg="green")
    click.echo()
